...
/** 
 ...
 YYModel 使用如下这些抽象类型，在NSObject分类基础之上，将JSON解析成实体类对象，其类库主要组成结构
 
 YYClassInfo 对Class的抽象
 
 YYClassIvarInfo 对Class的每一个Ivar的抽象
 YYClassMethodInfo 对Class的每一个Method的抽象
 YYClassPropertyInfo 对Class的每一个Property的抽象
 _YYModelMeta 对YYClassInfo对象的一个描述类，模仿MetaClass的效果，承载其Class映射的一些具体信息
 
 _YYpropertyMeta 对YYClassPropertyInfo对象的一个描述类，承载其Property映射的一些具体信息
 
 NSObject分类，提供了一些让NSObject实体类重写来自己配置一些jsonkey与属性映射的规则
 
 实体类哪一些属性不需要被json解析
 个性化配置 属性与jsonkey 的映射（不相同的时候）
 数组属性的Class实体类
 针对某一个json dict 映射一个单独的Class实体类
 针对某一个解析的json dict 返回给实体类自己检查是否合法
 当一个json dict不能被解析时 回调实体类自己处理，并返回一个可以被解析的json dict
 NSObject分类，提供了 -[实体类 jsonToModel:json] json转实体的便捷入口，
 并在该方法中拿到实体类的Class，继而解析Class成ClassModel对象，以及其属性与jsonkey的映射.
